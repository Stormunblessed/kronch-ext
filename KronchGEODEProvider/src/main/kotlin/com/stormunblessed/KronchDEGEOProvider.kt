...
class KronchDEGEOProvider: MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val urls = listOf(
            Pair("$krunchyapi/content/v1/browse?locale=de-DE&n=30&sort_by=popularity", "Beliebt"),
            Pair("$krunchyapi/content/v1/browse?locale=de-DE&n=30&sort_by=newly_added", "Neu hinzugefügt")
        )
        getKronchGEOToken()
        println("COUNTRY $latestcountryID")
        urls.apmap {(url, name) ->
            val res = app.get(url,
    ...
    private suspend fun getMovie(id: String?):ArrayList<Episode> {
        ...
        val metadainfo = app.get("$krunchyapi/content/v2/cms/movie_listings/$id/movies?locale=de-DE", headers = latestKrunchyHeader).parsed<KrunchyLoadMain>()
        metadainfo.data.map {
            val title = it.title
            val epID = it.id
    ...
    private suspend fun getRecommendations(seriesId: String?): List<SearchResponse>?{
        getKronchGEOToken()
        val recsurl = "$krunchyapi/content/v2/discover/similar_to/$seriesId?locale=de-DE&n=30"
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val tttt = if (isMovie) "$krunchyapi/content/v2/cms/movie_listings/$seriesIDSuper?locale=de-DE" else "$krunchyapi/content/v2/cms/series/$seriesIDSuper?locale=de-DE"
        ...
        if (tvType == TvType.Anime) {
            ...
            val nn =
                app.get(
                    "${krunchyapi}/content/v2/cms/series/$seriesIDSuper/seasons?locale=de-DE",
                    headers = latestKrunchyHeader
                )
            ...
            val inn =
                nn.data.filter {
                    ...
                    // == "de-DE" || it.audioLocale?.isEmpty() == true
                }
            val innversions =
                inn.filter {
            ...
            inn.apmap { main ->
                ...
                val res =
                    app.get(
                        "${krunchyapi}/content/v2/cms/seasons/$mainID/episodes?&locale=de-DE",
                        headers = latestKrunchyHeader
                    )
                        .parsedSafe<BetaKronch>()
                        ?: throw ErrorLoadingException("Couldn't get episodes, try again")
            ...
            innversions.map { ve ->
                val versionsfiltered =
                    ve.versions?.filter {
                        (it.audioLocale?.contains(Regex("ja-JP|de-DE|zh-CN")) == true ||
                ...
                versionsfiltered?.apmap { vers ->
                    ...
                    val resv =
                        app.get(
                            "${krunchyapi}/content/v2/cms/seasons/$guid/episodes?&locale=de-DE",
                            headers = latestKrunchyHeader
                        )
                            .parsedSafe<BetaKronch>()
                            ?: throw ErrorLoadingException("Couldn't get episodes, try again")
    ...
    override suspend fun loadLinks(
        ...
        links.map {testt ->
            ...
            bbb.apmap { aa ->
                aa.entries.filter {
                    it.key == "de-DE" || it.key.isEmpty()
                }.map {
                    it.value
                }.apmap {
                    val raw = it.hardsubLocale?.isEmpty()
                    val hardsubinfo = it.hardsubLocale?.contains("de-DE")
                    val vvv = if (it.url!!.contains("vrv.co")) "_VRV" else ""
                    val name = if (raw == false && issub) "Kronch$vvv Hardsub Deutsch (DE)" else if (raw == true && issub) "Kronch$vvv RAW" else "Kronch$vvv Deutsch"
